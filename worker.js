...
const guess = () => fetch('http://www.weather.com.cn/forecast/').then(r => r.text()).then(content => {
  const hrefs = content.split('http://api.map.baidu.com/').slice(1).map(s => {
    return 'http://api.map.baidu.com/' + s.split('&q;')[0].replaceAll('&a;', '&');
  });
  ...
  if (near) {
    return fetch(near).then(r => r.json()).then(j => {
      if (j && j?.location?.stationId.length) {
        return 'http://www.weather.com.cn/forecast/' + j.location.stationId[0];
      }
    });
  }
});
...
const update = async reason => {
  ...
  if (prefs.url && prefs.url !== 'http://www.weather.com.cn/forecast/') {
